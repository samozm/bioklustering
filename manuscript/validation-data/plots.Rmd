...
tmpdat = read.table("../scripts/gmm/predictions_10%_only0s_pre_bestModel.csv", header=TRUE, sep=",")
str(tmpdat)
tmpdat2 = read.table("../scripts/gmm/pca_10%_only0_pre.csv", header=TRUE, sep=",")
str(tmpdat2)
dat$pred10.0.pre = tmpdat$Labels
dat$pred10.0.pre.x1 = tmpdat2$principal.Component.1
dat$pred10.0.pre.x2 = tmpdat2$Principal.Component.2

tmpdat3 = read.table("../datasets/labels_ten_percent_only0s.csv", header=TRUE, sep=",")
tmpdat3
dat$labs10.0.given = tmpdat3$Labels
...
p4 = ggplot(dat,aes(x=pred10.0.pre.x1, y=pred10.0.pre.x2, color=as.factor(pred10.0.pre),shape=as.factor(target)))+geom_point(cex=4, alpha=0.65)+
   xlab("PC1") + ylab(" ")+ ggtitle("GMM 10% labels (only 0's) - Pre-Processing")+
   theme(
    plot.title = element_text(hjust=0.5, size=rel(1.6)),
    axis.title.x = element_text(size=rel(1.4)),
    axis.title.y = element_text(size=rel(1.4), angle=90, vjust=0.5, hjust=0.5),
...
p5 = ggplot(dat[dat$labs10.0.given == 0,],aes(x=pred10.0.pre.x1, y=pred10.0.pre.x2, color=as.factor(pred10.0.pre),shape=as.factor(target)))+geom_point(cex=4, alpha=0.65)+
   xlab("PC1") + ylab(" ")+ ggtitle("GMM 10% labels (only 0's) - Pre-Processing")+
   theme(
    plot.title = element_text(hjust=0.5, size=rel(1.6)),
    axis.title.x = element_text(size=rel(1.4)),
...
pdf("gmm-10-0-pre.pdf",width=5,height=5)
p4
dev.off()

pdf("gmm-10-0-pre-only-labeled.pdf",width=5,height=5)
p5
dev.off()
```
